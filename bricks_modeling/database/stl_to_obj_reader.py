...
from bricks_modeling.database.ldr_to_stl_reader import get_file_name_in_a_directory_with_suffix
...
def convert_stl_to_obj(stl_file_name, stl_directory, obj_directory, debug):
    ...
    if debug:
        if stl_file_name in template_ids:
            if stl_file_name in get_file_name_in_a_directory_with_suffix(stl_directory, ".obj"):
    ...
    if (
        ...
        and stl_file_name not in get_file_name_in_a_directory_with_suffix(stl_directory,'.obj')
    ):
        stl2obj(f'{stl_directory + "/" + stl_file_name + ".stl"}', f'{obj_directory + "/" + stl_file_name + ".obj"}')
        print(f"new file{stl_file_name}.obj has been created")

def convert_stls_to_objs(stl_directory, obj_directory, debug=False):
    for stl_file_name in get_file_name_in_a_directory_with_suffix(stl_directory, '.stl'):
...
if __name__ == "__main__":
    ...
    stl_single_parts_direcotry = os.path.join(
        path.dirname(__file__), "stl", "s"
    )
