...
=======
    ...
    items = ["d_x", "d_y", "d_z"] + [f"sin_phi_{m}" for m in "xyz"] + [f"cos_phi_{m}" for m in "xyz"]
    ...
    sqaure_sum_exprs = [
        ...
        vars[i, f"cos_phi_{m}"] * vars[i, f"cos_phi_{m}"] == 1
    ...
    def _matmul(a, b):
        bT = list(zip(*b))
        return [
            [_dot(a[i], bT[j]) for j in range(len(b))]
...
>>>>>>> Stashed changes
    ...
    for edge in graph.edges:
        ...
        edge_energy: grb.QuadExpr = sum([(m - n) * (m - n) for m, n in zip(world_a, world_b)])
        energy += edge_energy
