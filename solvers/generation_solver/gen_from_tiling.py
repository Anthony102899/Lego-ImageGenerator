...
def find_from_middle(tiling):
    ...
    sketch = tiling.sketch
    base_num = tiling.base_num
    ...
    for i in range(start_ring_idx, num_rings):
        ...
        for last_ring_idx in range(start, last_ring_num):
            ...
            for align_tile in tile_set:
                ...
                neighbour_tiles = generate_all_neighbor_tiles(base_brick=last_brick, align_tile=align_tile, color=i + 1, sketch=sketch, base_num=base_num)
        ...
        write_bricks_to_file(
            result_tiles, file_path=debugger.file_path(f"n={len(result_tiles)} r={i+1}.ldr"))
    return result_tiles

if __name__ == "__main__": #os.path.join(os.path.dirname(__file__), f'super_graph/last={last_ring_idx} r={i}#{num_rings}.pkl')
    tiling = structure_graph = pickle.load(open(os.path.join(os.path.dirname(__file__), f'super_graph/r=1#2 True.pkl'), "rb"))
    num_rings = tiling.num_rings
