...
class MinizincSolver(BaseSolver):
    ...
    def solve(self, structure_graph, node_sd, node_area, node_weight, base_count):
        ...
        instance["from_collision"] = [int(edge + 1) for edge in overlap_edges[:, 0]] if len(
            overlap_edges.shape) > 1 else []
        instance["to_collision"] = [int(edge + 1) for edge in overlap_edges[:, 1]] if len(
            overlap_edges.shape) > 1 else []
        instance["from_connect"] = [int(edge + 1) for edge in connect_edges[:, 0]] if len(
            connect_edges.shape) > 1 else []
        instance["to_connect"] = [int(edge + 1) for edge in connect_edges[:, 1]] if len(connect_edges.shape) > 1 else []
        instance["touch_length"] = [float(round(tlen, 2)) for tlen in connect_edges[:, 2]] if len(
            connect_edges.shape) > 1 else []
        instance["perimeter"] = [float(round(brick.template.perimeter * 25, 2)) for brick in
                                 structure_graph.bricks] if len(structure_graph.bricks) > 0 else []
