...
def get_file_name_in_a_directory(file_dir) -> []:
    ...
    for files in os.listdir(file_dir):
        if (
            os.path.splitext(files)[1] == ".dat" or os.path.splitext(files)[1] == ".stl"
        ):  # Get file name end wtih dat or stl
...
def convert_ldr_to_stl(ldr_file_name, ldr_directory, stl_directory, debug):
    ...
    if (
        ...
        and ldr_file_name not in get_file_name_in_a_directory(stl_directory)
    ):
        os.system(
...
if __name__ == "__main__":
    ...
    LEGO_single_parts_direcotry = os.path.join(
        path.dirname(__file__), "ldraw", "parts", "s"
    )
